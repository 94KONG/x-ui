...
func (s *Server) startTask() {
	...
	// Проверяйте каждые 30 секунд, работает ли xray
	s.cron.AddJob("@every 30s", job.NewCheckXrayRunningJob())

	go func() {
		time.Sleep(time.Second * 5)
		// Трафик подсчитывается каждые 10 секунд, первая задержка запуска 5 секунд, время перезапуска xray разнесено
		s.cron.AddJob("@every 10s", job.NewXrayTrafficJob())
	}()

	// Проверять избыток и истечение входящего трафика каждые 30 секунд
